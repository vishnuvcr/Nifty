...
def page(strategy,signals,ledger,site_dir):
    ...
    rows="".join(f"<tr><td>{html.escape(str(r.signal_id))}</td><td>{html.escape(str(r.expiry))}</td><td>{html.escape(str(r.status))}</td><td>{html.escape(str(r.signal))}</td><td>{'' if pd.isna(r.mc_ev_points_net) else f'{float(r.mc_ev_points_net):.2f}'}</td><td>{'' if pd.isna(r.realized_pnl_inr) else f'₹{float(r.realized_pnl_inr):,.2f}'}</td></tr>" for _,r in signals.tail(25).iloc[::-1].iterrows())
