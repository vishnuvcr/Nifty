...
def strategy_page(strategy,signals,ledger,latest,site_dir):
    ...
    dd=float((eq-eq.cummax()).min()) if len(eq) else 0.0
    def val(key,default="—"):
        x=latest.get(key,default)
        if x is None or (isinstance(x,float) and np.isnan(x)): return default
        return str(x)
    ev="" if val("mc_ev_points_net","") in ("","nan") else f'{float(latest.get("mc_ev_points_net")):.2f}'
    pop="" if val("mc_pop","") in ("","nan") else f'{float(latest.get("mc_pop")):.1%}'
    spot="" if val("spot","") in ("","nan") else f'{float(latest.get("spot")):,.2f}'
    rows=[]
    if not signals.empty:
        for _,r in signals.tail(12).iloc[::-1].iterrows():
            ...
            rows.append(f'<tr><td>{html.escape(str(r.get("decision_date","—")))}</td><td>{html.escape(str(r.get("expiry","—")))}</td><td>{html.escape(str(r.get("status","—")))}</td><td>{html.escape(str(r.get("signal","—")))}</td><td>{rv or "—"}</td><td>{rp or "—"}</td></tr>')
...
<title>NIFTY {title} Paper Trading</title>
<style>
...
<p><a href="../index.html">← NIFTY defined-risk selector</a></p>
<div class="card"><span class="badge">NIFTY • {title.upper()} • PAPER ONLY</span>
<h1>{title} Paper-Trading Scanner</h1>
<p class="note">Frozen NIFTY prospective validation. Entry at 09:30 IST; 5,000 Monte Carlo paths; 756-session lookback; net MC-EV gate; expiry settlement benchmark. No retrospective strategy ranking or parameter retuning.</p></div>
<div class="card"><h2>Latest signal</h2>
<p><span class="badge">{html.escape(val("signal","NO DATA YET"))}</span></p>
<div class="grid">
<div><small>Decision date</small><div class="kpi">{html.escape(val("decision_date"))}</div></div>
<div><small>Fixed entry time</small><div class="kpi">09:30 IST</div></div>
<div><small>Model cutoff</small><div class="kpi">{html.escape(val("model_data_cutoff"))}</div></div>
<div><small>Quote snapshot</small><div class="kpi">{html.escape(val("quote_retrieved_at_ist"))}</div></div>
<div><small>Expiry</small><div class="kpi">{html.escape(val("expiry"))}</div></div>
...
<p class="note">The page is populated automatically from the frozen NIFTY prospective scanner. Missing executable quote/data fields produce NO_TRADE.</p></div>
...
<tr><td>Execution stress</td><td>2.0 adverse option points per executed contract</td></tr>
...
<p class="note">Prospective validation only. Historical limited-risk results are provenance and are not backfilled into this ledger. No future outcome changes this frozen strategy definition. Sell Put and Bull Put Spread are outside the active prospective universe.</p></div>
<div class="card"><small>Branch: research/prospective-defined-risk-nifty-v1. Paper-only publication; no broker order is submitted.</small></div>
</main></body></html>"""
